...
// 캐슬링을 위한 변수
	...
	function moveCheck(){
		...
		var numId = id * 1;												// id숫자로 변환
		var tmp; 														//움직일 자리 임시 저장(function parameter로 이용하기 위해)
		
		
		if(pieceName == "pawn"){											// 기물 이름이 pawn 일때
			pawnMove(numId, tmp, count);
		} else if (pieceName == "rook"){									
			rookMove(numId, tmp, count);
		} else if (pieceName.endsWith("knight")){	
			nightMove(numId, tmp, count)
		} else if (pieceName == "bishop"){
			bishopMove(numId, tmp, count, 0);
		} else if (pieceName == "queen"){
			count = rookMove(numId, tmp, count, 0);
			bishopMove(numId, tmp, count, 0);
		} else if (pieceName == "king"){
			count = rookMove(numId, tmp, count, 1);
			bishopMove(numId, tmp, count, 1);
		} else{
			alert("imp");
		}
	...
	function move(){											// 기물 위치 옮기고 원래 있던 자리 빈칸으로
		...
		for(var i = 0; i < moveable.length; i++){
			if(id == moveable[i]){
				...
				$("#" + moveable[i]).html("<img id = 'see' src = " + piecePath + ">");	
				$("#" + pre).html("<img src = ' '>");
				return true;
			}			
		}
	...
	function mapId(id){
		...
		var numId = id * 1;
		var FronN= String.fromCharCode((numId%10)+64);
		var BackN= Math.floor((numId/10)-9)*-1;	
		var result = FronN+""+BackN;	
		var $div = $("<div>"+"<img src="+piecePath+" id='wow'> " +result+"</div>");
		$div.appendTo($('#result'));
	...
	function pawnMove(numId, tmp, count){
	...
	function rookMove(numId, tmp, count, flag){
	...
	function bishopMove(numId, tmp, count, flag){
	...
	function nightMove(numId, tmp, count){
		var knightCount1 = -1;
		var knightCount2 = -2;
	...
	function init(){
		...
		for(var i = 1; i < 9;i++){
			$("#2" + i).html("<img id='see' src = 'resources/images/Bpawn.png'>")
			$("#7" + i).html("<img id='see' src = 'resources/images/Wpawn.png'>")		
		}		
		
		$("#11").html("<img id='see' src='resources/images/Brook.png' >")
		$("#18").html("<img id='see' src='resources/images/Brook.png'>")
		$("#81").html("<img id='see' src='resources/images/Wrook.png'>")
		$("#88").html("<img id='see' src='resources/images/Wrook.png'>")
			
		
		$("#12").html("<img id='see' src='resources/images/BLknight.png'>")
		$("#17").html("<img id='see' src='resources/images/BRknight.png'>")
		$("#82").html("<img id='see' src='resources/images/WLknight.png'>")
		$("#87").html("<img id='see' src='resources/images/WRknight.png'>")
		
		$("#13").html("<img id='see' src='resources/images/Bbishop.png'>")
		$("#16").html("<img id='see' src='resources/images/Bbishop.png'>")
		$("#83").html("<img id='see' src='resources/images/Wbishop.png'>")
		$("#86").html("<img id='see' src='resources/images/Wbishop.png'>")
		
		$("#14").html("<img id='see' src='resources/images/Bqueen.png'>")
		$("#15").html("<img id='see' src='resources/images/Bking.png'>")
		$("#84").html("<img id='see' src='resources/images/Wqueen.png'>")
		$("#85").html("<img id='see' src='resources/images/Wking.png'>")	
	}
